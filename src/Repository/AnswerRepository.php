...
class AnswerRepository extends ServiceEntityRepository
{
	...
	public function findNoteStatsBySurvey(Survey $survey): array
	{
		$sql = <<<'SQL'
			...
			WHERE question.survey_id = :survey AND (question.type = :text OR question.type = :string)
	...
	public function findChoiceStatsBySurvey(Survey $survey): array
	{
		$sql = <<<'SQL'
			...
			FROM answer
			JOIN question ON question.id = answer.question_id
			WHERE question.survey_id = :survey AND (question.type = :radio OR question.type = :checkbox)
	...
	{
		$sql = <<<'SQL'
			SELECT question_id, scale_value, count(*) AS amount, sum(count(*)) OVER(partition by question_id) as total
			FROM answer
			JOIN question ON question.id = answer.question_id
			WHERE question.survey_id = :survey AND question.type = :scale
			GROUP BY question_id, scale_value
		SQL;
		$rsm = (new ResultSetMapping())
			->addScalarResult('id', 'id')
			->addScalarResult('question_id', 'question_id')
